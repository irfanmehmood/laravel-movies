...
@section('description', explode('.', $actor['biography'])[0])
@section('image', 'https://image.tmdb.org/t/p/w500/'.  $actor['profile_path'])
@section('content')
        ...
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <span class="ml-2">{{ $actor['birthday'] }} | Born in {{ $actor['place_of_birth'] }}</span>
                </div>
                ...
                <button 
                    x-data="{ url: '{{ url()->previous() }}' }"
                    x-on:click="window.location.href = url;"
                    class="mt-10 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Go back
                </button>

                <h2 class="text-4xl font-semibold mt-12">Known For</2>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    @foreach ($credits['cast'] as $movie)
                        @if (isset($movie['poster_path']))
                            <div class="mt-4">
                                <a href="{{route('movie.show', $movie['id']) }}"><img src="{{ 'https://image.tmdb.org/t/p/w500/'. $movie['poster_path'] }}" alt="poster" class="hover:opacity-75 transition ease-in-out duration-150"></a>
                                <a href="{{ $movie['id'] }}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1"></a>
                            </div>
                        @endif
                    @endforeach
                </div>
    ...
    @if (count($credits['crew']) > 1)
        <div class="credits border-b border-gray-800">
            <div class="container mx-auto px-4 py-16">
                <h2 class="text-4xl font-semibold">Credits</h2>
                <ul class="list-disc leading-loose pl-5 mt-8">
                    @foreach ($credits['crew'] as $credit)
                        @if (isset($credit['release_date']) && isset($credit['title']) && $credit['release_date'] != '')
                        <li>
                                {{ $credit['release_date'] }} &middot;
                                <strong><a href="{{ route('movie.show', $credit['id']) }}" class="hover:underline">{{ $credit['title'] }}</a></strong>
                        </li>
                        @endif
                    @endforeach
