        ...
        <div class="container mx-auto px-4 flex flex-col md:flex-row items-center justify-between px-4 py-6">
            <ul class="flex flex-col md:flex-row items-center">
                ...
                <li class="md:ml-16 mt-3 md:mt-0">
                    <a 
                        href="{{ route('movie.popular') }}" class="{{ Route::getCurrentRoute()->getActionName() == 'App\Http\Controllers\MovieController@popular' ? 'text-orange-400' : '' }} hover:text-gray-300"
                    >
                    Popular</a>
                </li>
                <li class="md:ml-6 mt-3 md:mt-0">
                    <a 
                        href="{{ route('movie.upcoming') }}" 
                        class="{{ Route::getCurrentRoute()->getActionName() == 'App\Http\Controllers\MovieController@upcoming' ? 'text-orange-400' : '' }} hover:text-gray-300"
                    >
                    Upcoming</a>
                </li>
                <li class="md:ml-6 mt-3 md:mt-0">
                    <a 
                        href="{{ route('movie.playing') }}" class="{{ Route::getCurrentRoute()->getActionName() == 'App\Http\Controllers\MovieController@playing' ? 'text-orange-400' : '' }} hover:text-gray-300"
                    >
                    Playing</a>
                </li>
                <li class="md:ml-6 mt-3 md:mt-0">
                    <a href="{{ route('movie.toprated') }}" class="{{ Route::getCurrentRoute()->getActionName() == 'App\Http\Controllers\MovieController@toprated' ? 'text-orange-400' : '' }} hover:text-gray-300">Top rated</a>
                </li>
            </ul>
